...
public class StarfangedDestroyerTail extends StarfangedDestroyerBody {
    public StarfangedDestroyerTail() {
    }

    private StarfangedDestroyerBody getNextBodyPart(int count) {
        StarfangedDestroyerBody next = (StarfangedDestroyerBody)this.next;

        for(int i = 0; i < count && next.next != null; ++i) {
            next = (StarfangedDestroyerBody) next.next;
        }
        return next;
    }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        if (this.isVisible()) {
            ...
            StarfangedDestroyerBody next = this.getNextBodyPart(2);
            if (next != null) {
