...
import deathrite.Projectiles.ShootingStarStariteBowProjectile;
...
public class StariteBow extends BowProjectileToolItem {
    ...
    protected void addExtraBowTooltips(ListGameTooltips tooltips, InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        tooltips.add(Localization.translate("itemtooltip", "staritebowtip"));
    }
    ...
    protected void fireProjectiles(Level level, int x, int y, PlayerMob player, InventoryItem item, int seed, ArrowItem arrow, boolean consumeAmmo, PacketReader contentReader) {
        ...
        Projectile ridium3Projectile = new ShootingStarStariteBowProjectile(player, player.x, player.y, (float)x, (float)y, (float)this.getProjectileVelocity(item, player) * arrow.speedMod, this.getAttackRange(item), this.getAttackDamage(item).add((float)arrow.damage, arrow.armorPen, arrow.critChance), this.getKnockback(item, player));
